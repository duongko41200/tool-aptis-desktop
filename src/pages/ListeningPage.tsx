...
function AudioPlayer({ playing, onToggle }: { playing: boolean; onToggle: () => void }) {
  return (
    <div style={{
      aspectRatio: '16 / 9', width: '100%', borderRadius: 'var(--r-lg)', overflow: 'hidden',
      ...
      <div style={{ position: 'relative', textAlign: 'center', color: '#fff' }}>
        ...
        <div className="text-shadow" style={{ marginTop: 14, fontSize: 14, fontWeight: 700 }}>
          Audio · Daily routine
        </div>
        <div className="text-shadow" style={{ fontSize: 12, opacity: 0.8 }}>
...
export default function ListeningPage() {
  ...
  const pick = (i: number) => { setActive(i); setChecked(false); setDict(''); };
  ...
  const result = (() => {
    ...
    return {
      correct,
      total: tw.length,
      words: tw.map((w, idx) => ({ w, ok: uw[idx] === w })),
    };
  })();
    ...
    <div className="screen">
      ...
      <div style={{ position: 'absolute', inset: 0, paddingTop: 92, display: 'grid', placeItems: 'center' }}>
        <div style={{
          ...
          display: 'grid',
          gridTemplateColumns: showT ? '1fr 360px' : '1fr',
          gap: 16,
          transition: 'grid-template-columns 280ms var(--ease)',
        }}>

          {/* ── LEFT: player + dictation ─────────────────────────────────── */}
          <div className="glass scroll" style={{
            ...
            <div style={{
              ...
              padding: '16px 20px', borderBottom: '1px solid var(--glass-edge)',
              position: 'sticky', top: 0, background: 'var(--glass)', zIndex: 2,
            }}>
                ...
                <div className="glass-2" style={{ display: 'flex', gap: 4, padding: 4, borderRadius: 'var(--r-pill)' }}>
                  {(['video', 'audio'] as const).map((k) => (
                    <button key={k} onClick={() => setMedia(k)} style={{
                      ...
                      color: media === k ? 'var(--accent-ink)' : 'var(--ink-2)',
                      background: media === k ? 'var(--accent)' : 'transparent',
                      transition: 'all 160ms var(--ease)',
                    }}>
                      <Icon name={k === 'video' ? 'video' : 'music'} size={14} />
                      {k === 'video' ? 'Video' : 'Audio'}
                    </button>
                  ))}
                </div>

                <button onClick={() => setShowT((s) => !s)} className="chip" style={{ fontSize: 12.5 }}>
                  <Icon name="list" size={14} />
                  {showT ? 'Ẩn lời thoại' : 'Hiện lời thoại'}
                </button>
              </div>
            </div>
            ...
            <div style={{ padding: 20, display: 'flex', flexDirection: 'column', gap: 18 }}>
              ...
              {media === 'video' ? (
                <div style={{
                  ...
                  background: '#000', boxShadow: 'var(--sh-md)',
                }}>
                  <iframe
              ...
              <div className="glass-2" style={{ padding: 18 }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
                  <div className="label-cap" style={{ color: 'var(--accent-deep)', whiteSpace: 'nowrap' }}>
                    <Icon name="pencil" size={13} /> Chép chính tả
                  </div>
                  <span className="chip" style={{ fontSize: 11.5, fontFamily: 'var(--font-mono)' }}>
                    Câu {active + 1}/{TRANSCRIPT.length} · {fmt(target.t)}
                  </span>
                </div>
                <p style={{ fontSize: 12.5, color: 'var(--ink-3)', margin: '0 0 10px' }}>
                  Nghe câu được chọn và gõ lại chính xác những gì bạn nghe được.
                </p>
                <textarea
                  value={dict}
                  onChange={(e) => { setDict(e.target.value); setChecked(false); }}
                  ...
                  style={{
                    ...
                    background: 'rgba(255,255,255,0.65)', padding: '11px 13px',
                    fontSize: 15, lineHeight: 1.5, fontFamily: 'var(--font)',
                ...
                {result && (
                  <div style={{
                    marginTop: 10, padding: '11px 13px', borderRadius: 'var(--r-sm)',
                    background: result.correct === result.total
                      ? 'rgba(111,174,90,0.14)' : 'rgba(224,169,59,0.12)',
                    border: `1px solid ${result.correct === result.total
                      ? 'rgba(111,174,90,0.4)' : 'rgba(224,169,59,0.35)'}`,
                  }}>
                    <div style={{
                      fontSize: 12.5, fontWeight: 800,
                      color: result.correct === result.total ? '#4e8a3c' : '#b5811f',
                      marginBottom: 6,
                    }}>
                    ...
                    <div style={{ fontSize: 14.5, lineHeight: 1.6 }}>
                      {result.words.map((x, i) => (
                        <span key={i} style={{
                          ...
                          fontWeight: x.ok ? 600 : 700,
                          textDecoration: x.ok ? 'none' : 'underline',
                        }}>
                          {x.w}{' '}
                        </span>
                      ))}
                ...
                <div style={{ display: 'flex', gap: 9, marginTop: 12 }}>
                  <button onClick={() => setPlaying(true)} className="btn btn-soft btn-sm">
                    <Icon name="volume" size={15} /> Nghe lại câu
                  </button>
                  <button onClick={() => togglePin(active)} className="btn btn-soft btn-sm">
                    <Icon name="bookmark" size={15} fill={pinned.includes(active)} />
                    {pinned.includes(active) ? 'Bỏ gim' : 'Gim câu'}
                  </button>
                  <button
              ...
              <div className="scroll" style={{ flex: 1, overflowY: 'auto', padding: 14 }}>
                ...
                {pinned.length > 0 && (
                  <div style={{ marginBottom: 12 }}>
                    <div className="label-cap" style={{ color: 'var(--accent-deep)', margin: '0 4px 8px' }}>
                      <Icon name="bookmark" size={12} fill /> Câu đã gim
                    </div>
                    {pinned.slice().sort((a, b) => a - b).map((i) => (
                      <button key={i} onClick={() => pick(i)} style={{
                        width: '100%', textAlign: 'left', display: 'flex', gap: 9,
                        padding: '9px 11px', borderRadius: 'var(--r-sm)',
                        background: 'rgba(217,232,157,0.4)',
                        border: '1px solid rgba(170,203,79,0.4)',
                        marginBottom: 6, cursor: 'pointer',
                      }}>
                        <span style={{
                          fontSize: 11, fontFamily: 'var(--font-mono)',
                          color: 'var(--accent-deep)', fontWeight: 700, paddingTop: 2,
                        }}>
                          {fmt(TRANSCRIPT[i].t)}
                        </span>
                        <span style={{ fontSize: 13, color: 'var(--ink)', lineHeight: 1.45, fontWeight: 600 }}>
                          {TRANSCRIPT[i].en}
                        </span>
                      </button>
                    ))}
                    <hr className="divider" style={{ margin: '12px 4px 0' }} />
                  </div>
                )}
                  ...
                  return (
                    <div
                      ...
                      style={{
                        ...
                        background: on ? 'rgba(217,232,157,0.45)' : 'transparent',
                        border: on ? '1px solid var(--accent-strong)' : '1px solid transparent',
                    ...
                    >
                      ...
                      <span style={{
                        ...
                        lineHeight: 1.5, fontWeight: on ? 600 : 500,
                      }}>
                        {line.en}
                      </span>
                      <button
                        ...
                        title="Gim câu"
                        style={{
                          color: pinned.includes(i) ? 'var(--accent-deep)' : 'var(--ink-3)',
