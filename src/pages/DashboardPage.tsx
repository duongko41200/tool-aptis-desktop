...
function ModeCard({ ic, tag, title, desc, meta, path, delay }: {
  ic: string; tag: string; title: string; desc: string; meta: string; path: string; delay?: string;
}) {
  ...
  return (
    <button
      ...
      style={{
        ...
        boxShadow: hover ? 'var(--sh-lg)' : 'var(--sh-md), inset 0 1px 0 rgba(255,255,255,0.55)',
        border: hover ? '1px solid var(--accent-strong)' : '1px solid var(--glass-line)',
        width: '100%',
      }}
    >
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <span style={{ width: 48, height: 48, borderRadius: 'var(--r-md)', display: 'grid', placeItems: 'center', background: 'var(--accent)', color: 'var(--accent-ink)', boxShadow: 'var(--sh-glow)' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr', gap: 16 }}>
          <div className="glass rise" style={{ padding: 22, animationDelay: '220ms' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
              ...
              <button className="chip" style={{ fontSize: 12 }} onClick={() => navigate('/writing/feedback')}>
