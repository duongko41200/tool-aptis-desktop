...
interface Message {
  from: 'ai' | 'me';
  text: string;
  corr?: { wrong: string; right: string; note: string };
}
...
const USER_LINES = [
  ...
  { text: "Yesterday I watched a movie with my friends.", corr: undefined },
];
  ...
  return (
    <div style={{ display: 'flex', justifyContent: isMe ? 'flex-end' : 'flex-start', gap: 10 }} className="rise">
      ...
      <div style={{ maxWidth: '76%' }}>
        <div style={{ padding: '12px 16px', borderRadius: isMe ? '18px 18px 6px 18px' : '18px 18px 18px 6px', background: isMe ? 'var(--accent)' : '#fff', color: isMe ? 'var(--accent-ink)' : 'var(--ink)', border: isMe ? 'none' : '1px solid var(--glass-edge)', boxShadow: 'var(--sh-sm)', fontSize: 14.5, fontWeight: 500, lineHeight: 1.5 }}>
          {m.text}
        </div>
        {m.corr && (
...
export default function AIPracticeTab() {
  ...
  const [topic, setTopic] = useState<string>('weekend');
  ...
  const speak = () => {
    if (rec) {
      ...
      setRec(false);
      setTimeout(() => {
        setMsgs(m => [...m, { from: 'ai', text: REPLIES[turn % REPLIES.length].text }]);
      }, 700);
      setTurn(x => x + 1);
    } else {
      setRec(true);
    ...
    <div style={{ width: '100%', height: '100%', display: 'grid', gridTemplateColumns: '1fr 320px', gap: 16 }}>
      ...
      <div className="glass" style={{ display: 'flex', flexDirection: 'column', overflow: 'hidden', borderRadius: 'var(--r-xl)' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 11 }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 12, color: 'var(--good)', display: 'flex', alignItems: 'center', gap: 5 }}>
                <span style={{ width: 7, height: 7, borderRadius: '50%', background: 'var(--good)' }} /> Đang lắng nghe
              </div>
            </div>
          </div>
          ...
          {rec && (
            <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
              <div className="chip chip-accent" style={{ gap: 8 }}>
                <span className="pulse-soft" style={{ width: 8, height: 8, borderRadius: '50%', background: 'var(--bad)' }} /> Đang ghi âm…
              </div>
            </div>
          )}
        </div>
        ...
        <div style={{ padding: 18, borderTop: '1px solid var(--glass-edge)', display: 'flex', alignItems: 'center', gap: 14, justifyContent: 'center' }}>
          <button className="iconbtn" style={{ background: 'rgba(40,55,30,0.07)', color: 'var(--ink)', border: '1px solid var(--glass-edge)' }} title="Bàn phím">
          ...
          <button className="iconbtn" style={{ background: 'rgba(40,55,30,0.07)', color: 'var(--ink)', border: '1px solid var(--glass-edge)' }} title="Nghe lại">
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16, minHeight: 0 }}>
        <div className="glass" style={{ padding: 18 }}>
          ...
          {([
            ['weekend', 'Kể về cuối tuần',    'Hằng ngày'],
            ['hobby',   'Sở thích của bạn',   'Hằng ngày'],
            ['travel',  'Chuyến đi đáng nhớ', 'Deep talk'],
          ] as const).map(([k, label, cat]) => (
            <button key={k} onClick={() => setTopic(k)}
              style={{ width: '100%', textAlign: 'left', display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px', borderRadius: 'var(--r-sm)', marginBottom: 6, transition: 'all 140ms var(--ease)', background: topic === k ? 'rgba(217,232,157,0.5)' : 'transparent', border: topic === k ? '1px solid var(--accent-strong)' : '1px solid transparent' }}>
