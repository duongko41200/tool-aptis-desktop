...
const CEFR_CONFIG: Record<CefrLevel, { color: string; bg: string; label: string }> = {
...
function CefrBadge({ level, active, isTarget }: { level: CefrLevel; active: boolean; isTarget?: boolean }) {
...
export default function B2CriteriaResult({ result, targetLevel = 'B2' }: Props) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <div style={{
        ...
        background: cefrCfg.bg, border: `1px solid ${cefrCfg.color}44`,
        ...
        <div style={{ flex: 1, minWidth: 120 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4, flexWrap: 'wrap' }}>
            <span style={{ fontSize: 15, fontWeight: 800, color: cefrCfg.color }}>
              {cefrLevel} — {cefrCfg.label}
            </span>
            {meetsTarget ? (
              <span style={{
                fontSize: 11, fontWeight: 700, padding: '2px 8px', borderRadius: 'var(--r-pill)',
                background: 'rgba(39,174,96,0.15)', color: 'var(--good)',
              }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
        ...
        <div style={{ flex: 1 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
            <span style={{ fontSize: 15, fontWeight: 800, color: 'var(--ink)' }}>Chất lượng ngôn ngữ</span>
